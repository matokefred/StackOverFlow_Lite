...
#  Customize any errors to come back in json and not in HTML
    ...
    @app.route('/apiv1/questions/<int:id>', methods=['GET'])
    def get_question(id):
        qn = [qn for qn in questions if qn['id'] == id]
        if len(qn) == 0:
            abort(400)
    ...
    @app.route('/apiv1/questions/<int:id>/answers', methods=['POST'])
    def add_answer(id):
        qn = [qn for qn in questions if qn['id'] == id]
        if len(qn) == 0:
            abort(400)
    ...
    @app.route('/apiv1/questions/<int:id>', methods=['DELETE'])
